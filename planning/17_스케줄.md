...
- [ ] 5계층 + Remote Gateway 아키텍처 합의, 일반 셸 경로 / AI 경로 분리 확정 — `→ 03_프로젝트_아키텍처_정의서.md 참조`
...
| **Phase 3 — Team & Enterprise** | Phase 2 이후 | **마일스톤 R0/RA/P3-1~3로 상세화(아래 §4.1)**. 현 상태 릴리즈, remote-approval 완주(PWA), signed policy.d·트러스트 채널, 중앙 감사·팀 프로파일·엔터프라이즈 마스킹, MCP mutate/external·OAuth, gVisor·Guardrails 고도화 |
...
> Phase 4의 상세 주차 계획은 Phase 3 안정화 후 회고를 거쳐 확정한다.

### 4.1 Phase 3 마일스톤 (상세화 2026-06-05)

> 구현 정본: `→ (terminal repo) docs/superpowers/specs/2026-06-05-phase3-roadmap-design.md`, 추적 백로그: `→ (terminal repo) docs/TASK.md` Phase 3 절. 순서(가치 우선): **R0 → RA → P3-1 → (P3-2 ∥ P3-3)**. 플랫폼: Linux x86_64 + Windows 네이티브(macOS 제외), 동적 감시·gVisor는 Linux 우선.

| 마일스톤 | 핵심 영역 | 완료 기준(DoD 요약) | 근거 |
|---|---|---|---|
| **R0 — 현 상태 릴리즈(v0.2.0)** | 패키징·배포 | Linux x86_64 + Windows 네이티브 바이너리(feature 매트릭스), install 스크립트, GitHub Release + SHA256 체크섬, 크로스빌드 CI | §29.11 |
| **RA — remote-approval 완주** | 원격 승인(PWA) | 디바이스 리스너+페어링(TOFU)+게이트 결선(fail-closed)+context_hash → 실데몬 + PWA(/approve·/pair) 실폰 승인. relay(M2)는 완주 후 재평가 | §28, §30-12/13 |
| **P3-1 — 트러스트 채널 + 조직 정책** | 서명·정책 | ed25519 공통 trust channel(정책·플러그인·스킬·바이너리), signed policy.d(readonly·최우선·monotonic), 스킬 서명·조직 레지스트리 | §30-7, §30-9, §29.11, §26.6 |
| **P3-2 — 중앙 감사 + 팀 프로파일 + 마스킹** | 거버넌스 | 감사 로그 export(명령 미전송), 팀별 프로파일, 엔터프라이즈 마스킹, Debug Bundle(마스킹 강제) | §12.3, §29.5 |
| **P3-3 — MCP 확장 + 고격리/가드레일** | 격리·통합 | MCP mutate/external 컨센트 + OAuth(OS keyring), Guardrails 동적 감시(seccomp/cgroups/eBPF Linux), gVisor 샌드박스 | §30-8, §30-10, §30-11 |

> Phase 3 주차 단위 일정은 R0 릴리즈 후 피드백을 반영해 마일스톤 착수 시점에 확정한다.
